...
interface AssetsProps extends BoxProps {
  assets: (AssetProps & { _type: string })[];
  colorScheme: keyof Theme["colors"];
}

function Content({ colorScheme = "current", content }: AssetProps) {
  return (
    <ContentComponent colorScheme={colorScheme} content={content!} my={6} />
  );
}

function File({ file }: AssetProps) {
...
function Image({ image }: AssetProps) {
...
function Video({ video }: AssetProps) {
...
function Assets({ colorScheme, assets }: AssetsProps) {
  return (
    <div>
      {assets.map((asset, index) => {
        ...
        return (
          <AssetController key={index} colorScheme={colorScheme} {...asset} />
        );
      })}
    </div>
  );
